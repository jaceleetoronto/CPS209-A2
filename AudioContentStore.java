...
public class AudioContentStore
{
    ...
    public AudioContentStore(){
        contents = readContentFromFile("store.txt");
    }

    private ArrayList<AudioContent> readContentFromFile(String fileName) {
        ...
        try {
            ...
            while (in.hasNextLine()) {
                ...
                if (line.equals("SONG")) {
                    ...
                    Song song = new Song(title, year, id, Song.TYPENAME, lyrics.toString(), length, artist, composer, genre, lyrics.toString());
                    ...
                    AudioBook audiobook = new AudioBook(title, year, id, AudioBook.TYPENAME, chapters.toString(), length, author, narrator, chapterTitles, chapters);
                    audioContents.add(audiobook);
                }
            }
        } catch (IOException e) {
...
//        contents.add(book);
    ...
    public ArrayList<Integer> titleMap(String title)
    {
        Map<String, ArrayList<Integer>> titleMap = new TreeMap<String, ArrayList<Integer>>();
        ArrayList<Integer> indices = new ArrayList<Integer>();
        for (int i = 0; i < contents.size(); i++) {
            String contentTitle = contents.get(i).getTitle().toLowerCase();
            if (!titleMap.containsKey(contentTitle)) {
                titleMap.put(contentTitle, new ArrayList<>());
            } titleMap.get(contentTitle).add(i+1);
        }
        if (titleMap.containsKey(title.toLowerCase())) {
            indices= titleMap.get(title.toLowerCase());
        } return indices;
    }

    public ArrayList<Integer> artistMap(String artist) {
        Map<String, ArrayList<Integer>> artistMap = new TreeMap<String, ArrayList<Integer>>();
        ArrayList<Integer> indices = new ArrayList<Integer>();
        for (int i = 0; i < contents.size(); i++) {
            if (contents.get(i).getType() == Song.TYPENAME) {
                ...
                if (!artistMap.containsKey(songArtist)) {
                    artistMap.put(songArtist, new ArrayList<Integer>());
                } artistMap.get(songArtist).add(i+1);
                ...
                if (!artistMap.containsKey(author)) {
                    artistMap.put(author, new ArrayList<>());
                } artistMap.get(author).add(i+1);
            }
        }
        if (artistMap.containsKey(artist.toLowerCase())) {
            indices = artistMap.get(artist.toLowerCase());
        }
        return indices;
    }
    public ArrayList<Integer> genreMap(String genre)
    {
        Map<String, ArrayList<Integer>> genreMap = new TreeMap<>();
        ArrayList<Integer> indices = new ArrayList<>();
        for (int i = 0; i < contents.size(); i++) {
            if (contents.get(i).getType() == Song.TYPENAME) {
                Song song = (Song) contents.get(i);
                String songGenre = song.getGenre().toString().toLowerCase();
                if (!genreMap.containsKey(songGenre)) {
                    genreMap.put(songGenre, new ArrayList<Integer>());
                }
                genreMap.get(songGenre).add(i+1);
            }
        }
        if (genreMap.containsKey(genre.toLowerCase())) {
            indices = genreMap.get(genre.toLowerCase());
        }
        return indices;
    }
